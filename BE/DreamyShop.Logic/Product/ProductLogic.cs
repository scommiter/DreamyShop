...
namespace DreamyShop.Logic.Product
{
    public class ProductLogic : IProductLogic
    {
        ...
        {
            var totalCount = await EFCoreExtensions.CountAsync(_context.Products);
            ...
            parameter.Add(new SqlParameter("@num", pagingRequest.Limit * pagingRequest.Page));
            parameter.Add(new SqlParameter("@limit", pagingRequest.Limit));
            var productNews = _context.Set<ProductProcDto>().FromSqlRaw(@"EXEC GetProductPagings @num, @limit", parameter.ToArray());
            var productNewsList = await productNews.ToListAsync();
            ...
            var productsPaging = await EFCoreExtensions.ToListAsync(groupedQuery);

            var products = productsPaging.Select(x => new ProductDto
            {
                Id = x.ProductId,
                Name = x.Product.Name,
                Code = x.Product.Code,
